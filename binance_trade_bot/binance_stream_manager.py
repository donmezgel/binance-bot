...
class OrderGuard:
    ...
    def __enter__(self):
        try:
            if self.tag is None:
                raise Exception("OrderGuard düzgün ayarlanmamış")
...
class BinanceStreamManager:
        ...
        for (symbol, order_id) in pending_orders:
            ...
            while True:
                try:
                    ...
                    self.logger.error(f"Beklemeyen emri çekerken hata oluştu: {e}")
            ...
            self.logger.info(f"{symbol} için beklemede olan emir idsi {order_id} yakalandı:\n{fake_report}", False)
    ...
    def _process_stream_data(self, stream_data):
        ...
        if event_type == "executionReport":  # !userData
            ...
            self.logger.debug(f"Bakiye güncellemesi: {stream_data}")
            with self.cache.open_balances() as balances:
            ...
            self.logger.error(f"Bilinmeyen bir event oluştu: {event_type}\n{stream_data}")
