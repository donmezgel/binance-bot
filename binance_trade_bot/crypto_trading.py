...
def main():
    logger = Logger()
    logger.info("Bismillah.. Başlıyoruz..")
    ...
    try:
        ...
        logger.error("Binance APIye erişemiyorum")
        logger.error(e)
        return
    strategy = get_strategy(config.STRATEGY)
    if strategy is None:
        logger.error("Geçersiz strateji ismi")
        return
    trader = strategy(manager, db, logger, config)
    logger.info(f"Seçilen strateji: {config.STRATEGY}")

    logger.info("Veritabanı şeması oluşturuyorum..")
    db.create_database()
