...
class AutoTrader:
    ...
    def transaction_through_bridge(self, pair: Pair):
        ...
        if balance and balance * from_coin_price > self.manager.get_min_notional(
            ...
            self.logger.info("Satış pas geçiliyor..")

        if can_sell and self.manager.sell_alt(pair.from_coin, self.config.BRIDGE) is None:
            self.logger.info("Satamadım, keşif moduna geri dönüyorum...")
            return None
        ...
        self.logger.info("Satın alamadım, keşif moduna geri dönüyorum...")
        return None
    ...
    def update_trade_threshold(self, coin: Coin, coin_price: float):
        ...
        if coin_price is None:
            self.logger.info("Güncelleme pas geçiliyor... mevcut coin {} bulunamadı".format(coin + self.config.BRIDGE))
            return
        ...
        with self.db.db_session() as session:
            for pair in session.query(Pair).filter(Pair.to_coin == coin):
                ...
                if from_coin_price is None:
                    self.logger.info(
                        "Güncelleme pas geçiliyor... mevcut coin {} bulunamadı".format(pair.from_coin + self.config.BRIDGE)
                    )
                    continue
        ...
        with self.db.db_session() as session:
            for pair in session.query(Pair).filter(Pair.ratio.is_(None)).all():
                if not pair.from_coin.enabled or not pair.to_coin.enabled:
                    continue
                self.logger.info(f"Başlatılıyor {pair.from_coin} vs {pair.to_coin}")

                from_coin_price = self.manager.get_ticker_price(pair.from_coin + self.config.BRIDGE)
                if from_coin_price is None:
                    self.logger.info(
                        "Geçiş başlatılıyor {}, sembol bulunamadı".format(pair.from_coin + self.config.BRIDGE)
                    )
                    continue
                ...
                if to_coin_price is None:
                    self.logger.info(
                        "Geçiş başlatılıyor {}, sembol bulunamadı".format(pair.to_coin + self.config.BRIDGE)
                    )
                    continue
    ...
    def _get_ratios(self, coin: Coin, coin_price):
        ...
        for pair in self.db.get_pairs_from(coin):
            ...
            if optional_coin_price is None:
                self.logger.info(
                    "Keşif modu pas geçiliyor... opsiyonel coin {} bulunamadı".format(pair.to_coin + self.config.BRIDGE)
                )
                continue
    ...
    def _jump_to_best_coin(self, coin: Coin, coin_price: float):
        ...
        if ratio_dict:
            ...
            self.logger.info(f"Bu coinden {coin} diğerine atlıyorum {best_pair.to_coin_id}")
        ...
        for coin in self.db.get_coins():
            ...
            if not any(v > 0 for v in ratio_dict.values()):
                ...
                if bridge_balance > self.manager.get_min_notional(coin.symbol, self.config.BRIDGE.symbol):
                    self.logger.info(f"Köprü koinini kullanarak bu coini {coin} satın alıyorum")
