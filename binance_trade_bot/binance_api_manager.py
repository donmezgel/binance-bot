...
class BinanceAPIManager:
    ...
    def get_ticker_price(self, ticker_symbol: str):
        ...
        if price is None and ticker_symbol not in self.cache.non_existent_tickers:
            ...
            price = self.cache.ticker_values.get(ticker_symbol, None)
            if price is None:
                self.logger.info(f"Coin bulunamadı: {ticker_symbol} - bundan sonra bu coinle işlem yapılmayacak")
    ...
    def retry(self, func, *args, **kwargs):
        for attempt in range(20):
            try:
                ...
                self.logger.warning(f"Al/Sat hata verdi. Tekrar deniyorum (attempt {attempt}/20)")
                if attempt == 0:
    ...
    def _wait_for_order(
        ...
        while True:
            ...
            if order_status is not None:
                break
            self.logger.debug(f"{order_id} nolu emrin oluşmasını bekliyorum..")
            time.sleep(1)
        ...
        while order_status.status != "FILLED":
            try:
                ...
                self.logger.debug(f"{order_id} nolu emrin tamamlanmasını bekliyorum")
                ...
                if self._should_cancel_order(order_status):
                    ...
                    if order_status.status == "PARTIALLY_FILLED" and order_status.side == "BUY":
                        self.logger.info("Satış kısmi olarak yapıldı")
                    ...
                    self.logger.info("Keşif moduna geri dönüyorum...")
                    return None

                if order_status.status == "CANCELED":
                    self.logger.info("Emir iptal edildi, keşif moduna geri dönüyorum...")
                    return None
                ...
                self.logger.info(f"Beklenmeyen hata: {e}")
                time.sleep(1)
    ...
    def _buy_alt(self, origin_coin: Coin, target_coin: Coin):  # pylint: disable=too-many-locals
        ...
        while order is None:
            try:
                ...
                self.logger.warning(f"Beklenmeyen hata: {e}")
        ...
        self.logger.info(f"{origin_symbol} satın aldım...")
    ...
    def _sell_alt(self, origin_coin: Coin, target_coin: Coin):  # pylint: disable=too-many-locals
        ...
        order_quantity_s = "{:0.0{}f}".format(order_quantity, pair_info["baseAssetPrecision"])
        self.logger.info(f"{order_quantity} adet {origin_symbol} satıyorum..")
